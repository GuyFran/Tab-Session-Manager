...
| --- | --- | --- | --- |
| DBG-01 | Remove the temporary `DEBUG_RESTORE_TAB_LIMIT = 10` cap in `src/background/open.js` (the constant, the capping block in `createTabs()`, the warn/trace block in `openSession()`, the two trace events, and the debug-panel banner) once the incognito restore-routing investigation is finished | P1 | ☐ **Active in v7.4.21** — every restored window currently stops after 10 tabs. Set the constant to `0` to disable without removing the code. |
