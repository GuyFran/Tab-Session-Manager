...
| PH-01 | Incognito placeholder URLs can exceed Chrome's own session-file limit. Chrome persists each tab navigation as a session command capped at ~63 KB (`CreateUpdateTabNavigationCommand` → `SerializedNavigationEntry::WriteToPickle`, which writes an **empty** URL when the entry would not fit). A 1600 px JPEG of a real page is often 80–200 KB, i.e. 100–270 KB once base64-encoded into the data:URL, so those tabs would come back **blank after a Chrome restart** ("continue where you left off" / recently-closed). Not yet reproduced — found by code reading on 2026-09-16 while diagnosing the 800-tab sweep crash. Fix options: cap the embedded thumbnail so the whole URL stays under ~60 KB (lower quality/width until it fits), or keep the thumbnail out of the URL. | P1 | ☐ **Open** — verify first: restart Chrome with a placeholder whose URL is > 63 KB and check whether the tab restores with its URL |
| SCALE-01 | Run a real 800-tab sweep on v7.4.57 to confirm the crash is gone. v7.4.57 removed the per-tab full-window `tabs.query` (was quadratic), capped concurrent windows at 2 (`preloadSweepMaxParallelWindows`), throttled the debug trace to 1 s, and made the popup's window list stop dumping every tab every 3 s. None of it was exercised at 800 tabs — user report 2026-09-16, fix untested at scale. | P1 | ☐ **User verification** — reload the unpacked extension, sweep one window first, then "Sweep all" |
