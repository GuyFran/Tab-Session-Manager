...
| --- | --- | --- | --- |
| DBG-01 | Remove the temporary `DEBUG_RESTORE_TAB_LIMIT = 10` cap in `src/background/open.js` (and its `budget` plumbing, the two trace events, and the debug-panel banner) once the incognito restore-routing investigation is finished | P1 | ☐ **Active in v7.4.20** — every restore currently stops after 10 tabs per session. Set the constant to `0` to disable without removing the code. |
