...
| SCALE-01 | Run a real 800-tab sweep on v7.4.58 to confirm the crash is gone. v7.4.57 removed the per-tab full-window `tabs.query` (was quadratic), capped concurrent windows at 2 (`preloadSweepMaxParallelWindows`), throttled the debug trace to 1 s, and made the popup's window list stop dumping every tab every 3 s. v7.4.58 caps every placeholder URL under 60 KB (PH-01). None of it was exercised at 800 tabs — user report 2026-09-16, fix untested at scale. While checking, also confirm PH-01: after the sweep, `sweep-tab-swapped` events should show `placeholderBytes` < 60000 and no `sweep-placeholder-trimmed` events; then restart Chrome and confirm hibernated incognito tabs come back with their URL. | P1 | ☐ **User verification** — reload the unpacked extension, sweep one window first, then "Sweep all" |
...
| Date | Change |
| --- | --- |
| 2026-09-16 | **v7.4.59 — completion notifications for restore and sweep (user request).** New `background/notify.js` posts OS notifications through the `notifications` API (permission added to both manifests; the unpacked extension may need to be re-enabled once after reload because of the new permission). **Restore:** "Session restored — "name" — N windows, M tabs in T s" at the end of `openSession()` (windows = actually created/used, tabs = tab count of the saved session), or "Session restore failed — "name" — error" from the catch. **Sweep:** "Sweep finished" / "Sweep stopped" — "N tabs across W windows in T — K new thumbnails, C already cached"; per-run counters (`processed`, `captured`, `cachedSkips`) feed a module-level total that is emitted once when no sweep is active anymore, so "Sweep all" plus deferred resumes produce one message, not one per window; "Nothing to sweep" when every checked window was skipped. Setting **`ifNotifyOnRestoreAndSweep`** (default on, options page under "Open") gates both; the guard treats a missing key as on. Also: the post-capture "stored?" check in the sweep now uses `hasThumbnail()` instead of building the full base64 data URL three times per tab. **Dev build 7.4.59: 0 errors, 29 Sass-loader deprecation warnings.** Not exercised in a browser. |
| 2026-09-16 | **v7.4.58 — PH-01 fixed: every incognito placeholder URL stays under 60 KB.** Chrome writes each tab navigation to its session file as one command capped at ~63 KB (`CreateUpdateTabNavigationCommand` → `SerializedNavigationEntry::WriteToPickle` → `WriteStringToPickle`, which stores an **empty** URL when it does not fit), so a placeholder whose data:URL exceeded that came back blank after a Chrome restart. Three layers now enforce the cap. **(1) Capture:** `thumbnails.js` replaces the fixed 1600 px / q0.6 encode with `encodeToFit()`, a ladder of (width, quality) steps — 1600/.6, 1600/.5, 1280/.5, 1280/.4, 1024/.45, 1024/.4, 800/.45, 800/.4, 640/.4, 480/.4, 320/.35 — taking the first JPEG ≤ **36,000 bytes** (`MAX_PLACEHOLDER_THUMBNAIL_BYTES`, = 48,000 base64 chars). **(2) Legacy store:** `getThumbnailDataUrl()` re-encodes any stored blob over the cap through the same ladder on first use and writes it back (keeping its date), so pre-7.4.58 thumbnails (up to 3,000 of them) migrate lazily — `thumb-shrunk` trace event. **(3) Builder:** `buildIncognitoPlaceholder()` (new; `buildIncognitoPlaceholderUrl` is now a thin wrapper) measures the URL the way Chrome stores it — `specByteLength()` counts non-ASCII as UTF-8 bytes × 3 because GURL percent-escapes them in data:URL paths and fragments — against **`MAX_PLACEHOLDER_URL_BYTES` = 60,000**, and if over drops the favicon, then the thumbnail, then shortens the title (200-char cap, 60 in the last resort); the real URL and title always survive. Data: favicons are also capped at 4 KB in the body. Both callers (sweep swap, restore-from-cache in open.js) log `placeholderBytes`, and a `sweep-placeholder-trimmed` / trimmed fields in `tab-placeholder-from-cache` when anything was dropped. **Verified (node, builder only):** shell overhead is 1,462 bytes; a maximum thumbnail + 200-char CJK title + http favicon = 54,848 bytes; + an 8 KB URL = 57,554 bytes; all fit and round-trip URL/title; a 150 KB legacy thumbnail is dropped rather than emitted oversize. **Verified (browser, ladder on synthetic 1920×1080 pages):** simple article 1600/.6 = 60,972 B (**already over the cap under the old fixed encode**) → fits at 1280/.4 = 34,256 B; dense text 424 KB → 480×270/.4 = 30,469 B; photo-heavy 238 KB → 480×270/.4 = 28,111 B; pure noise 449 KB → 21,372 B. So text-heavy real pages will hibernate at 480 px wide — the price of the 60 KB cap. Chrome restart behaviour itself not re-tested — folded into SCALE-01. **Debug panel (user request):** new summary tiles "Cached thumb, no reload" (sweep skipped the reload), "Restored from cache" (restore placed the tab straight onto a thumbnail placeholder), "Hibernated with thumb" / "Hibernated without thumb" (placeholders from restore + sweep, split by whether an image made it in), "Legacy thumbs shrunk", "Placeholder trimmed" and "Largest placeholder" (KB; red above 60 KB), plus an explainer paragraph when anything went without a thumbnail. |
| 2026-09-16 | **v7.4.57 — sweep no longer scales quadratically with tab count (user report: sweeping 800 tabs crashes Chrome).** Four costs all grew with tab count and ran for the whole sweep: (1) `sweepWindow()` re-queried the **entire window** (`tabs.query({windowId})`) before every single tab — with incognito placeholders each tab's data:URL is tens to hundreds of KB, so one query is tens of MB and the sweep serialized that n times (n² in tabs) through the browser process and the service worker, concurrently for every window; (2) "Sweep all" ran **every** window in parallel with no cap, each keeping 1–2 real pages loading; (3) the debug trace re-serialized its full 8,000-event ring buffer (~1 MB) to `storage.session` **and** broadcast it on a 75 ms debounce, i.e. up to 13×/s for an hour-long sweep; (4) the popup's "Open windows" panel dumped every tab (`windows.getAll({populate:true})`) every 3 s. Fixes: the sweep now takes **one** snapshot of target tab ids per pass and confirms each with `tabs.get(id)` (O(1) per tab), re-querying the window at most 3 times per sweep to catch ids changed by Memory Saver (`sweep-pass` event); the initial tab list from `startPreloadSweep()` is reused instead of fetched twice. New setting **`preloadSweepMaxParallelWindows`** (default 2, min 1, options page under "Open") caps concurrently swept windows across all invocations (manual + deferred resumes); the rest wait in a queue shown as "queued" in the popup with a working Stop, and queued windows are **not** placeholder-suppressed so user clicks still navigate. Debug persist/broadcast throttled to 1 s (the panel polls at 1 s anyway). The popup fetches all tabs only on open and every 15 s, and otherwise polls just windows + active tabs, carrying tab counts forward. **Dev build 7.4.57: 0 errors, 29 warnings, all Sass-loader deprecations** (the count moved 27→29 somewhere in 7.4.49–7.4.56, which were never re-verified). **Not exercised at 800 tabs** — see SCALE-01. Side finding logged as PH-01: Chrome's session file caps a navigation at ~63 KB and blanks the URL beyond it, which large placeholder URLs can exceed. |
| 2026-09-02 | **v7.4.54 — incognito tabs restore straight to hibernated placeholders.** Restored incognito tabs are placed directly onto their data:URL thumbnail placeholders with the cached thumbnail already shown, so no post-restore sweep is needed to reach the hibernated end state. _(Logged from commit `2607fe1` during the 2026-09-11 docs pass; build/runtime status not re-verified in this pass.)_ |
| 2026-09-01 | **v7.4.53 — reorder windows within a saved session.** Per-window up/down buttons let a saved session's windows be reordered, and the chosen order is honored on restore. _(Logged from commit `132d112` during the 2026-09-11 docs pass; build/runtime status not re-verified in this pass.)_ |
| 2026-09-01 | **v7.4.52 — Session Buddy v3 imports supported (collections format).** The v7.4.51 import log immediately paid off: a user export failed with "no 'sessions' property" — Session Buddy v3 switched to `{ collections: [ { title, folders: [ { links, state/top/left/width/height, incognito? } ] } ] }`. New `diagnoseSessionBuddy3`/`convertSessionBuddy3` in `ImportSessionsComponent.js`: each collection → one TSM session (date recovered from the "Sep 1, 2026 • 5:19 PM" title via Date.parse after stripping the bullet and unicode spaces, fallback now), each folder → a window (geometry kept in windowsInfo), each link → a tab. Since open.js derives a restored window's incognito state from its **tabs'** incognito flag (not windowsInfo), the folder's `incognito` is stamped on every tab; `active`/`pinned` pass through. Unrecognized-JSON errors now report all three diagnoses (TSM, SB, SB v3). Converter logic verified in node against both user samples (regular + incognito). |
| 2026-09-01 | **v7.4.51 — Import log panel in the options page, with Copy logs.** Below the Import section a persistent "Import log" panel now records every read/import event with millisecond timestamps: file read start (name, size), read OK (session/tab counts, duration) or read FAILED (the v7.4.50 diagnostic reason, in red), background send success, sendMessage split-and-retry, and per-session send failures (the old split recursion could loop forever on a single failing session — now guarded). The log lives in storage.local (`importDebugLog`, last 300 entries, survives page reloads), **Copy logs** puts the full text on the clipboard (execCommand fallback), **Clear** wipes it. The file input also resets after each pick so the same file can be re-selected after a failure. |
| 2026-09-01 | **v7.4.50 — import "Read failed" now says why.** The options-page import reader (`ImportSessionsComponent.js`) failed silently: every unrecognized file resolved `undefined` → the row showed only "Read failed", and an exception inside the FileReader callback (lz4 decompress, short `.session` file) left the promise unresolved so the row never appeared at all. Now every failure path resolves `{ error: reason }` and the row shows "Read failed — \<reason\>" (also `console.error`'d with the filename): JSON parse errors with position, TSM diagnosis (which session #, which missing keys), Session Buddy diagnosis (missing 'sessions', unknown type, missing keys), lz4 decompress/JSON/version errors, `.session` structural errors, unsupported extension, FileReader `onerror`. |
| 2026-09-01 | **v7.4.49 — merge sessions.** Combine several saved sessions into a single new session, with optional duplicate-tab cleanup. _(Logged from commit `2956482` during the 2026-09-11 docs pass; build/runtime status not re-verified in this pass.)_ |
| 2026-09-01 | **v7.4.48 — debug log is a ring buffer; counters never freeze.** A user export from a real 226-tab restore+sweep revealed that the fixed 2,000-event cap silently stopped ALL recording mid-sweep — including the summary counters, because they updated inside the same guarded path. appendEvent now always updates the summary and broadcasts; the events array keeps the most recent 8,000 (oldest dropped, eventsDropped counted and shown in the panel footer: "N oldest dropped — counters include them"). The same export otherwise showed a flawless run: 226/226 tabs, 0 blanks, 0 errors, group fully recreated, and 67 sweep tabs processed with 65 thumbnails and 54 regroups before the recorder froze. |
